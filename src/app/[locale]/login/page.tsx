...
import { OAuthProvider } from "@/lib/schemas/auth";
...
export default function LoginPage() {
  ...
  async function handleOAuthLogin(provider: OAuthProvider) {
    ...
    try {
      ...
      const res = await apiRoute.get<{ authUrl: string }>(`/auth/oauth?provider=${provider}&redirectTo=${encodeURIComponent(window.location.origin + `/${locale}/auth/callback`)}`);
      
      if (res.code === 200 && (res.data as { authUrl?: string }).authUrl) {
        window.location.href = (res.data as { authUrl: string }).authUrl;
      } else {
        throw new Error((res as unknown as { msg?: string }).msg || "获取登录链接失败");
      }
    } catch (error) {
      toast.error(`三方登录失败: ${error instanceof Error ? error.message : "未知错误"}`);
    } finally {
